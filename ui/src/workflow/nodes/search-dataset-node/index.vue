<template>
  <NodeContainer :nodeModel="nodeModel">
    ...
    <el-card shadow="never" class="card-never">
      ...
      >
        <el-form-item :label="$t('selectKnowledgeBase')">
          <template #label>
            <div class="flex-between">
              <span>{{ $t('selectKnowledgeBase') }}</span>
        ...
        <el-form-item :label="$t('searchParameters')">
          <template #label>
            <div class="flex-between">
          ...
          <div class="w-full">
            <el-row>
              <el-col :span="12" class="color-secondary lighter">{{ $t('searchMode') }}</el-col>
              <el-col :span="12" class="lighter">
                {{ $t(SearchMode[form_data.dataset_setting.search_mode as keyof typeof SearchMode]) }}
              </el-col>
              <el-col :span="12" class="color-secondary lighter">{{ $t('similarityAbove') }}</el-col>
              <el-col :span="12" class="lighter">
                {{ form_data.dataset_setting.similarity }}</el-col>
              <el-col :span="12" class="color-secondary lighter">{{ $t('referenceSegmentsTop') }}</el-col>
              <el-col :span="12" class="lighter"> {{ form_data.dataset_setting.top_n }}</el-col>
              <el-col :span="12" class="color-secondary lighter">{{ $t('maxReferenceCharacters') }}</el-col>
              <el-col :span="12" class="lighter">
        ...
        <el-form-item
          :label="$t('searchQueryInput')"
          prop="question_reference_address"
          :rules="{
            message: $t('selectSearchQueryInput'),
            trigger: 'blur',
            required: true
          }"
        >
          <NodeCascader
            ...
            :nodeModel="nodeModel"
            class="w-full"
            :placeholder="$t('selectSearchQueryInput')"
